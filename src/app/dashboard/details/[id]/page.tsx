...
import { TabletSmartphone, View, Timer, Calendar } from "lucide-react";
  ...
  const applyFilters = () => {
    ...
    if (filters.english_title) {
      filtered = filtered.filter((item) =>
        item.english_title
          .toLowerCase()
          .includes(filters.english_title.toLowerCase())
      );
    }

    if (filters.level) {
      filtered = filtered.filter(
        (item) => item.level === Number(filters.level) // Convert to number for comparison
      );
    }
      ...
      <div className="p-6">
        ...
        <div className="mb-6 grid grid-cols-2 md:grid-cols-6 gap-4">
          <input
            ...
            onChange={(e) =>
              setFilters({ ...filters, english_title: e.target.value })
            }
